...
impl VR {
    ...
    pub fn from_binary(chars: [u8; 2]) -> Option<Self> {
        from_utf8(chars.as_ref())
            ...
            .or_else(|| Self::from_nonstandard_shorthand(chars))
    }

    /// Some vendor DICOM writers emit their own internal "ambiguous VR"
    /// dictionary placeholder literally into the file instead of resolving
    /// it against PixelRepresentation, e.g. "xs" (lowercase) meaning "US or
    /// SS" (also used by dcmjs's dictionary, which resolves it the same way).
    /// "xs" is short-form (2-byte length) like the VR it stands in for, so
    /// treating it as unrecognized (which callers fall back to VR::UN for,
    /// a long-form VR) misparses the length field and desyncs the rest of
    /// the dataset. Resolve it to a concrete short-form VR so parsing can
    /// continue; US over SS is a best-effort default since the true
    /// signedness isn't recoverable from the placeholder alone.
    ///
    /// "ox" (lowercase, meaning "OB or OW") gets the same dcmjs-style
    /// resolution for VR-reporting consistency, though it isn't actually
    /// parse-breaking: OB and OW are both long-form, matching VR::UN's
    /// layout, so the fallback already reads the correct bytes - it just
    /// mislabels the VR.
    ///
    /// Other known dcmjs placeholders ("lt", "up", "na") are deliberately
    /// not resolved here: "lt" in particular spans both short-form (US/SS)
    /// and long-form (OW) VRs, so its on-disk length-field width can't be
    /// inferred from the placeholder alone - a wrong guess would desync the
    /// dataset rather than fix it. Add a case only once a real file
    /// demonstrates which width it actually used.
    fn from_nonstandard_shorthand(chars: [u8; 2]) -> Option<Self> {
        match &chars {
            b"xs" | b"XS" => Some(VR::US),
            b"ox" | b"OX" => Some(VR::OW),
            _ => None,
        }
    }
...
/// # Ok::<_, dcmnorm_core::header::ParseTagError>(())
/// ```
...
mod tests {
    ...
    /// test for the fix itself; `dcmnorm`'s own test suite additionally covers it end-to-end
    /// through full dataset parsing (`reads_element_with_nonstandard_ambiguous_vr_shorthand_*`
    /// in dcmnorm/src/dicom_io/tests.rs).
    #[test]
    fn vr_from_binary_resolves_dcmjs_ambiguous_vr_shorthand() {
        assert_eq!(VR::from_binary(*b"xs"), Some(VR::US));
        assert_eq!(VR::from_binary(*b"XS"), Some(VR::US));
        assert_eq!(VR::from_binary(*b"ox"), Some(VR::OW));
        assert_eq!(VR::from_binary(*b"OX"), Some(VR::OW));

        // Deliberately unresolved placeholders (see from_nonstandard_shorthand's doc
        // comment for why "lt" specifically can't be guessed safely) still fall through.
        assert_eq!(VR::from_binary(*b"lt"), None);
        assert_eq!(VR::from_binary(*b"up"), None);
        assert_eq!(VR::from_binary(*b"na"), None);

        // Standard VR codes are unaffected by the fallback.
        assert_eq!(VR::from_binary(*b"US"), Some(VR::US));
