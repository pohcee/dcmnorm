...
//!
//! This module allows you to roll your own [transfer syntax index][1] by
//! implementing [`TransferSyntaxIndex`] directly.
//!
...
#[macro_export]
/// Submit a transfer syntax specifier to be supported by the program's
/// runtime. No-op: dcmnorm registers its transfer syntaxes directly in
/// `dcmnorm-transcode`'s static built-in table rather than through a
/// plugin/inventory mechanism (removed - never used by anything in this
/// workspace). Kept as a no-op for API compatibility with any external code
/// still calling it.
macro_rules! submit_transfer_syntax {
    ($ts: expr) => {
        // ignore request - see doc comment above
    };
}

#[macro_export]
/// Submit an explicit VR little endian transfer syntax specifier to be
/// supported by the program's runtime. No-op, for the same reason as
