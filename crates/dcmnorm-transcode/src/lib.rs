...
//! but the pixel data may not be decoded through this crate. To include
//! support for some transfer syntaxes with encapsulated pixel data,
//! add the **`native`** Cargo feature
//! or one of the other image encoding features available.
//!
//! All registered TSes are readily available
...
//! provide implementations that are written in pure Rust (JPEG decoding via
//! the in-house `dcmnorm-jpeg`, encoding via `jpeg-encoder`) and are likely
//! available on all supported platforms without issues.
...
use dcmnorm_encoding::transfer_syntax::{AdapterFreeTransferSyntax as Ts, Codec};
