...
use dcmnorm_core::header::{DataElementHeader, HasLength, Length, VR};
use dcmnorm_core::value::{DicomValueType, PrimitiveValue};
use dcmnorm_core::{value::Value, DataElement, Tag};
...
impl IntoTokens for dcmnorm_core::header::EmptyObject {
    ...
    I: IntoTokens,
{
    ...
    Items(
        FlattenTokens<
            <dcmnorm_core::value::C<AsItem<I>> as IntoIterator>::IntoIter,
            ItemTokens<I::Iter>,
        >,
    ),
    ...
    PixelData(
        ...
        /// Option is used for easy taking from a &mut,
        /// should always be Some in practice
        Option<dcmnorm_core::value::C<P>>,
        /// Frame offset table
        OffsetTableItemTokens<dcmnorm_core::value::C<u32>>,
    ),
    /// the header and offset of encapsulated pixel data was read,
    /// fragments come next
    PixelDataFragments(
        FlattenTokens<
            <dcmnorm_core::value::C<ItemValue<P>> as IntoIterator>::IntoIter,
            ItemValueTokens<P>,
        >,
    ),
    /// no more elements
    ...
    fn next(&mut self) -> Option<Self::Item> {
        let (out, next_state) = match self {
                    ...
                    DataToken::SequenceStart { tag, len } => {
                        ...
                        match elem.into_value() {
                            ...
                            Value::Sequence(seq) => {
                                ...
                                let items: dcmnorm_core::value::C<_> = seq
                                    .into_items()
                                    .into_iter()
            ...
            DataElementTokens::PixelData(fragments, tokens) => {
                ...
                let tokens: dcmnorm_core::value::C<_> =
...
impl<T> IntoTokens for dcmnorm_core::value::C<T>
where
    T: IntoTokens,
{
    type Iter =
        FlattenTokens<<dcmnorm_core::value::C<T> as IntoIterator>::IntoIter, <T as IntoTokens>::Iter>;
...
mod tests {
    ...
    #[derive(Debug, Clone)]
    struct SimpleObject<T>(Length, dcmnorm_core::value::C<T>);
        ...
        T: HasLength,
    {
        type Iter = super::FlattenTokens<
            <dcmnorm_core::value::C<T> as IntoIterator>::IntoIter,
